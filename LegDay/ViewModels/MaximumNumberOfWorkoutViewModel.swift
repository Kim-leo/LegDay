...
class MaximumNumberOfWorkoutViewModel {
    ...
    func setMaximumNumberOfWorkoutAndReadyToLoadOnWorkoutVC(row: Int) {
        myWorkoutModel.setMaximumNumberOfWorkout = maximumNumberOfWorkout[row]
        print(myWorkoutModel.setMaximumNumberOfWorkout)
    }
    
    func showMaxNumberOnLabel(view: MaximumNumberOfWorkoutView, row: Int) {
        let maxNumber = Array(1...(row + 5)).reduce(0, +)
        view.maxNumLabel.text = "\(maxNumber)" + " 회"
        
        let totalNumber = maxNumber * 4
        view.totalNumberLabel.text = "\(totalNumber)" + " 회"
        
        
        let fontSize = UIFont.boldSystemFont(ofSize: 30)
        let attributedString = NSMutableAttributedString(string: view.maxNumLabel.text ?? "")
        attributedString.addAttribute(.font, value: fontSize, range: (view.maxNumLabel.text! as NSString).range(of: "\(maxNumber)"))
        attributedString.addAttribute(.foregroundColor, value: Colors().redColor, range: (view.maxNumLabel.text! as NSString).range(of: "\(maxNumber)"))
        ...
        attributedString2.addAttribute(.font, value: fontSize, range: (view.totalNumberLabel.text! as NSString).range(of: "\(totalNumber)"))
        attributedString2.addAttribute(.foregroundColor, value: Colors().softRedColor, range: (view.totalNumberLabel.text! as NSString).range(of: "\(totalNumber)"))
